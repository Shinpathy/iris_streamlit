...
#目標値を数字から花の名前に変更
df.loc[df['target']==0, 'target'] = 'setosa' #locメソッドで行と列を指定して入れ替え
...
#出力値をメインパネルに表示できるようにする
#予測値のデータフレーム
pred_probs = clf.predict_proba(value_df.values) #ロジスティック回帰による確率を取得できる。
pred_df = pd.DataFrame(pred_probs, columns=['setosa', 'versicolor', 'virginica'], index=['probability'])

st.write('##Prediction') #Predictionを書く
st.write(pred_df) #pred_df（データフレーム）をかく
